...
POWER_STATES = ["D0", "D1", "D2", "D3",
                "D3cold", "S3D3cold", "CLKSTOP", "EPSS"]
    ...
    def get_val_db(self, val):
        ...
        if val > self.nsteps:
            ...
            if val != 0 or self.nsteps != 0:
                print(("val > nsteps? for nid 0x%02x") %
                      self.nid, val, self.nsteps)
        else:
            db = off + val * range
        return db
...
class HDANode:
    ...
    def __init__(self, codec, nid, cache=True):
        ...
        if self.wtype_id == 'VOL_KNB':
            self.conn_list = True

        self.wcaps_list = []
        if self.stereo:
            self.wcaps_list.append('STEREO')
        if self.in_amp:
            self.wcaps_list.append('IN_AMP')
        if self.out_amp:
            self.wcaps_list.append('OUT_AMP')
        if self.amp_ovrd:
            self.wcaps_list.append('AMP_OVRD')
        if self.format_ovrd:
            self.wcaps_list.append('FORMAT_OVRD')
        if self.stripe:
            self.wcaps_list.append('STRIPE')
        if self.proc_wid:
            self.wcaps_list.append('PROC_WID')
        if self.unsol_cap:
            self.wcaps_list.append('UNSOL_CAP')
        if self.conn_list:
            self.wcaps_list.append('CONN_LIST')
        if self.digital:
            self.wcaps_list.append('DIGITAL')
        if self.power:
            self.wcaps_list.append('POWER')
        if self.lr_swap:
            self.wcaps_list.append('LR_SWAP')
        if self.cp_caps:
            self.wcaps_list.append('CP_CAPS')
    ...
    def set_active_connection(self, val):
        ...
        if self.active_connection != None:
            ...
            if not self.disable_reread:
                self.active_connection = self.codec.rw(
                    self.nid, VERBS['GET_CONNECT_SEL'], 0)
            else:
    ...
    def reread_pwr(self, value=None):
        ...
        self.pwr_setting_name = self.pwr_setting < 4 and POWER_STATES[
            self.pwr_setting] or "UNKNOWN"
    ...
    def reread(self):
        ...
        if self.in_amp:
            ...
            self.amp_vals_in = HDAAmpVal(
                self.codec, self, HDA_INPUT, self.amp_caps_in)
        if self.out_amp:
            self.amp_caps_out = HDAAmpCaps(self.codec, self.nid, HDA_OUTPUT)
            self.amp_vals_out = HDAAmpVal(
                self.codec, self, HDA_OUTPUT, self.amp_caps_out)
        if self.wtype_id == 'PIN':
            ...
            self.pincap = []
            if caps & (1 << 0):
                self.pincap.append('IMP_SENSE')
            if caps & (1 << 1):
                self.pincap.append('TRIG_REQ')
            if caps & (1 << 2):
                self.pincap.append('PRES_DETECT')
            if caps & (1 << 3):
                self.pincap.append('HP_DRV')
            if caps & (1 << 4):
                self.pincap.append('OUT')
            if caps & (1 << 5):
                self.pincap.append('IN')
            if caps & (1 << 6):
                self.pincap.append('BALANCE')
            if caps & (1 << 7):
                self.pincap.append('HDMI')
            if caps & (1 << 16):
                self.pincap.append('EAPD')
            if caps & (1 << 24):
                self.pincap.append('DP')  # display port
            if caps & (1 << 27):
                self.pincap.append('HBR')
            self.pincap_vref = []
            if caps & (1 << 8):
                self.pincap_vref.append('HIZ')
            if caps & (1 << 9):
                self.pincap_vref.append('50')
            if caps & (1 << 10):
                self.pincap_vref.append('GRD')
            if caps & (1 << 12):
                self.pincap_vref.append('80')
            if caps & (1 << 13):
                self.pincap_vref.append('100')
            self.reread_eapdbtl()
            ...
            self.defcfg_misc = []
            if caps & (1 << 8):
                self.defcfg_misc.append('NO_PRESENCE')
            ...
            if self.format_ovrd:
                ...
                self.pcm_streams = self.codec.analyze_pcm_streams(
                    self.pcm_stream)
        if self.proc_wid:
        ...
        if self.unsol_cap:
            unsol = self.codec.rw(
                self.nid, VERBS['GET_UNSOLICITED_RESPONSE'], 0)
        ...
        if self.codec.vendor_id == 0x10ec and self.nid == 0x20:
            self.realtek_coeff_proc = self.codec.rw(
                self.nid, VERBS['GET_PROC_COEF'], 0)
            self.realtek_coeff_index = self.codec.rw(
                self.nid, VERBS['GET_COEF_INDEX'], 0)
    ...
    def reread_eapdbtl(self, value=None):
        ...
        for name in EAPDBTL_BITS:
            ...
            if val & (1 << bit):
                self.pincap_eapdbtl.append(name)
    ...
    def eapdbtl_set_value(self, name, value):
        ...
        if value:
            ...
            self.pincap_eapdbtls &= ~mask
        self.codec.rw(
            self.nid, VERBS['SET_EAPD_BTLENABLE'], self.pincap_eapdbtls)
        self.reread_eapdbtl()
        return changed

    def reread_pin_widget_control(self, value=None):
        if value is None:
            pinctls = self.codec.rw(
                self.nid, VERBS['GET_PIN_WIDGET_CONTROL'], 0)
        else:
            pinctls = value
    ...
    def dig1_set_value(self, name, value):
        if name == 'category':
            ...
            self.codec.rw(
                self.nid, VERBS['SET_DIGI_CONVERT_2'], (self.digi1 >> 8) & 0xff)
        else:
            ...
            if value:
                ...
                self.digi1 &= ~mask
            self.codec.rw(
                self.nid, VERBS['SET_DIGI_CONVERT_1'], self.digi1 & 0xff)
        self.reread_dig1()
        return changed
    ...
    def revert(self, export=False):
        ...
        if not self.origin_pinctls is None:
            self.codec.rw(
                self.nid, VERBS['SET_PIN_WIDGET_CONTROL'], self.origin_pinctls)
        ...
        if not self.origin_pincap_eapdbtls is None:
            self.codec.rw(
                self.nid, VERBS['SET_EAPD_BTLENABLE'], self.origin_pincap_eapdbtls)
            self.reread_eapdbtl(self.origin_pincap_eapdbtls)
        if not self.origin_vol_knb is None:
            self.codec.rw(
                self.nid, VERBS['SET_VOLUME_KNOB_CONTROL'], self.origin_vol_knb)
            self.set_vol_knb(self.origin_vol_knb)
        if not self.origin_sdi_select is None:
            self.codec.rw(
                self.nid, VERBS['SET_SDI_SELECT'], self.origin_sdi_select)
            self.reread_sdi_select(self.origin_sdi_select)
        if not self.origin_digi1 is None:
            self.codec.rw(
                self.nid, VERBS['SET_DIGI_CONVERT_1'], self.origin_digi1 & 0xff)
            self.codec.rw(
                self.nid, VERBS['SET_DIGI_CONVERT_2'], (self.origin_digi1 >> 8) & 0xff)
    ...
    def export(self):
        ...
        active_connection = getit(
            'active_connection', self.origin_active_connection)
        ...
        if not self.origin_pincap_eapdbtls is None:
            self.codec.rw(
                self.nid, VERBS['SET_EAPD_BTLENABLE'], pincap_eapdbtls)
        ...
        if not self.origin_digi1 is None:
            ...
            self.codec.rw(
                self.nid, VERBS['SET_DIGI_CONVERT_2'], (digi1 >> 8) & 0xff)
    ...
    def get_mixercontrols(self):
        ...
        for ctl in ctls:
            for iface in (ctl.iface and [ctl.iface] or [None, 'card', 'pcm']):
                try:
                    id = AlsaMixerElemId(
                        iface=iface, name=ctl.name, index=ctl.index, device=ctl.device)
                    ...
                    e = None
            if not e:
                print(("Control ID not found:"),
                      ctl.dump_extra().strip().lstrip())
                continue
            e.hdactl = ctl
            res.append(e)
        return res
        ...
        if dst_node.in_amp:
            ...
            if dst_node.amp_vals_in.indices == dst_node.connections:
                if not self.nid in dst_node.connections:
                    raise ValueError("nid 0x%02x is not connected to nid 0x%02x (%s, %s)" % (
                        dst_node.nid, self.nid, repr(self.connections), repr(dst_node.connections)))
...
class HDACard:
    ...
    def __init__(self, card, ctl_fd=None):
        self.card = card
        if ctl_fd is None:
            self.fd = ctl_fd = os.open(
                "/dev/snd/controlC%i" % card, os.O_RDONLY)
        else:
            self.fd = os.dup(ctl_fd)
        info = struct.pack('ii16s16s32s80s16s80s128s', 0,
                           0, '', '', '', '', '', '', '')
...
class HDACodec:
        ...
        for i in range(conn_len):
            ...
            if range_val:
                if not prev_nid or prev_nid >= val:
                    raise IOError(
                        "invalid dep_range_val 0x%x:0x%x\n" % (prev_nid, val))
                n = prev_nid + 1
                while n <= val:
                    res.append(n)
                    n += 1
    ...
    def analyze(self):
        ...
        self.subsystem_id = self.param_read(
            AC_NODE_ROOT, PARAMS['SUBSYSTEM_ID'])
    ...
    def dump_node(self, node):
        ...
        def print_pin_caps(node):
            ...
            if 'IN' in node.pincap:
                str += " IN"
            if 'OUT' in node.pincap:
                str += " OUT"
            if 'HP_DRV' in node.pincap:
                str += " HP"
            if 'EAPD' in node.pincap:
                str += " EAPD"
            if 'PRES_DETECT' in node.pincap:
                str += " Detect"
            if 'BALANCE' in node.pincap:
                str += " Balance"
            if 'HDMI' in node.pincap:
                if (self.vendor_id >> 16) == 0x10ec:  # Realtek has different meaning
                    ...
                    if 'HBR' in node.pincap:
                        str += " HBR"
                    str += " HDMI"
            if 'DP' in node.pincap:
                str += " DP"
            if 'TRIG_REQ' in node.pincap:
                str += " Trigger"
            if 'IMP_SENSE' in node.pincap:
                str += " ImpSense"
            str += '\n'
            if node.pincap_vref:
                str += "    Vref caps:"
                if 'HIZ' in node.pincap_vref:
                    str += " HIZ"
                if '50' in node.pincap_vref:
                    str += " 50"
                if 'GRD' in node.pincap_vref:
                    str += " GRD"
                if '80' in node.pincap_vref:
                    str += " 80"
                if '100' in node.pincap_vref:
                    str += " 100"
                str += '\n'
            if 'EAPD' in node.pincap:
                str += "  EAPD 0x%x:" % node.pincap_eapdbtls
                if 'BALANCED' in node.pincap_eapdbtl:
                    str += " BALANCED"
                if 'EAPD' in node.pincap_eapdbtl:
                    str += " EAPD"
                if 'LR_SWAP' in node.pincap_eapdbtl:
                    str += " R/L"
                str += '\n'
            ...
            if node.pinctl:
                ...
                if 'IN' in node.pinctl:
                    str += " IN"
                if 'OUT' in node.pinctl:
                    str += " OUT"
                if 'HP' in node.pinctl:
                    str += " HP"
                if node.pincap_vref:
        ...
        def print_vol_knob(node):
            str = "  Volume-Knob: delta=%d, steps=%d, " % (
                node.vol_knb_delta, node.vol_knb_steps)
        ...
        def print_digital_conv(node):
            str = "  Digital:"
            if 'ENABLE' in node.dig1:
                str += " Enabled"
            if 'VALIDITY' in node.dig1:
                str += " Validity"
            if 'VALIDITYCFG' in node.dig1:
                str += " ValidityCfg"
            if 'EMPHASIS' in node.dig1:
                str += " Preemphasis"
            if 'COPYRIGHT' in node.dig1:
                str += " Copyright"
            if 'NONAUDIO' in node.dig1:
                str += " Non-Audio"
            if 'PROFESSIONAL' in node.dig1:
                str += " Pro"
            if 'GENLEVEL' in node.dig1:
                str += " GetLevel"
            str += "\n"
        ...
        str = "Node 0x%02x [%s] wcaps 0x%x:" % (
            node.nid, node.wtype_name(), node.wcaps)
        if node.stereo:
...
class HDA_Exporter_pyscript:
    ...
    def rw(self, old, nid, verb, param):
        if verb & 0x0800:
            raise ValueError(
                "read: nid=0x%x, verb=0x%x, param=0x%x" % (nid, verb, param))
...
if __name__ == '__main__':
    v = HDACodec()
    v.analyze()
    print("vendor_id = 0x%x, subsystem_id = 0x%x, revision_id = 0x%x" %
          (v.vendor_id, v.subsystem_id, v.revision_id))
    print("afg = %s, mfg = %s" % (v.afg and "0x%x" %
                                  v.afg or 'None', v.mfg and "0x%x" % v.mfg or 'None'))
    print()
    print()
    print(v.dump()[:-1])
